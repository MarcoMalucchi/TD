...
#define BUF_LEN 1024    //Dimensione buffer - Multiplo di 512 e 128 dopo aver letto 8 volte FIFO -> buffer RAM pieno
#define BLOCK_LEN 128   //Dati FIFO
...
// --- ISR accelerometro ---
void dataReadyAcc(void) {
  timeStamp[activeBuff][count] = micros();
  count ++;
  if (count == 128) {
    count = 0;
    fifoFlag = true;
...
void loop() {
  ...
  if (process.dataLogger) {
    ...
    if (tLogger > 30000000UL) loggerStop = true;
  }
  if (fifoFlag) {  //se data logger è attivo
    fifoFlag = false;
    uint16_t dataFIFO = readFIFOCount();
    if (DEBUG) Serial.println(dataFIFO);
    Serial.println(dataFIFO);
    if (dataFIFO < 768) {
      detachInterrupt(digitalPinToInterrupt(ACC_PIN)); //Disabilita interrupt
      Serial.println(F("FIFO corrotto -> reset FIFO"));
      setupMPU();
      writeMPU(0x6A, 0x40); // FIFO enable
      // writeMPU(0x6A, 0x44); // FIFO enable + FIFO reset
      // Interrupt accelerometro
      count = 0; activeBuff = 0;
      ...
      if (bufIndex >= BUF_LEN) {
        // Scrittura buffer
        size_t written = dataFile.write((uint8_t*)dataBuf, sizeof(dataBuf));
        if (DEBUG) {
          Serial.print(F("Byte scritti: "));
...
/*
  if (fifoFlag) {
    ...
    if (firstPass) {
      ...
      if (DEBUG) Serial.println(dataFIFO);
      if (dataFIFO > 768) {
        uint16_t extraData = dataFIFO/6-BLOCK_LEN;
        if (DEBUG) Serial.print(F("Extra data n. ")); Serial.println(extraData);
      	Wire.beginTransmission(MPU_ADDR);
	      Wire.write(0x74);delayMicroseconds(10);
	      Wire.endTransmission(false);
	      Wire.requestFrom(MPU_ADDR, 6*extraData);
	      for (uint16_t i = 0; i < 6*extraData; i++) { Wire.read();}
      }
      firstPass = false;
    }
    if readFIFOCount()!);
    uint16_t samplesToRead = BLOCK_LEN;
    for (uint16_t i = 0; i < samplesToRead; i++) {
      ...
      dataBuf[bufIndex].timeStamp = timeStamp[(int)!activeBuff][i];
      dataBuf[bufIndex].dataRaw.x_high = Wire.read(); dataBuf[bufIndex].dataRaw.x_low = Wire.read();
      dataBuf[bufIndex].dataRaw.y_high = Wire.read(); dataBuf[bufIndex].dataRaw.y_low = Wire.read();
      dataBuf[bufIndex].dataRaw.z_high = Wire.read(); dataBuf[bufIndex].dataRaw.z_low = Wire.read();
    ...
    Serial.print(bufIndex);Serial.print(F(" ")); Serial.println(block);
  }
  }
*/
//==============================================================================
...
void handleCommand(char *cmd) {
  ...
  if      (strcmp(cmd, "+++") == 0) {
    ...
    if (findNextDataFile("DATA", availableFile, sizeof(availableFile), targetFreq) == 0) {
      ...
      if (!dataFile.preAllocate(READFREQ_HZ * (MAX_TIME + 15)*sizeof(packet))) {
...
void setupMPU() {
  ...
  writeMPU(0x6B, 0x00);  delay(10);// wake up
